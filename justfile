...
#   --depth 2                the crate has 39 features. An uncapped powerset
#                            is a six-figure number of builds -- not slow,
#                            unrunnable, and the recipe never returned.
#                            Depth 2 is 590, and pairwise is where
...
# pull request, the powerset runs on a nightly schedule. 590 builds is too much
...
drivers:
    #!/usr/bin/env bash
    set -euo pipefail
    feats=api,api-docs,api-tokens,auth,cache,cli,crypt,database,dev-proxy,dx,events,i18n,inertia,jobs,macros,mail
