...
pub(crate) type TokenDb = crate::database::Driver;

/// The connection pool the store runs over -- the application's own pool.
pub type TokenPool = crate::database::Pool;
...
/// has one code path.
#[cfg(any(feature = "db-postgres", feature = "db-mysql"))]
pub(crate) fn restored_time(stored: StoredTime) -> Result<DateTime<Utc>, ApiTokenError> {
    Ok(stored)
}
...
#[cfg(feature = "db-sqlite")]
pub(crate) fn restored_time(stored: StoredTime) -> Result<DateTime<Utc>, ApiTokenError> {
    DateTime::from_timestamp_millis(stored).ok_or_else(|| ApiTokenError::Expiry(stored.to_string()))
}
