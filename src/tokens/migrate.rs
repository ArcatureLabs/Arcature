//! API token schema migrations.
//!
//! One embedded migration per dialect, applied under a lock where the dialect
//! has one, with an `arcature_api_tokens_schema_migrations` history table.
...
//!
//! This mirrors [`crate::jobs`]'s and [`crate::auth::session_store`]'s
//! migrators rather than sharing code with them, for the reason stated in the
//! latter: a generic migrator parameterised over table name, history table,
//! and lock would couple subsystems that have no reason to change together,
//! so that a schema change in one could break another's migration.

use sqlx::{Executor, Row};

use super::dialect::{TokenDb, TokenPool, sql};
use super::error::ApiTokenError;

/// The connection type of the dialect this build speaks.
type Conn = <TokenDb as sqlx::Database>::Connection;
...
const MIGRATIONS: &[(&str, &str)] = &[("0001_api_tokens", sql::SCHEMA)];

/// Apply all pending migrations over the pool.
///
/// # Errors
///
/// Returns [`ApiTokenError::Database`] if the database rejects a statement or
/// the connection fails.
pub(super) async fn apply(pool: &TokenPool) -> Result<(), ApiTokenError> {
...
/// them if it has one.
async fn apply_on(conn: &mut Conn) -> Result<(), ApiTokenError> {
...
async fn apply_pending(conn: &mut Conn) -> Result<(), ApiTokenError> {
...
mod tests {
    ...
    #[test]
    fn the_bundled_migration_creates_the_api_tokens_table() {
        ...
        for &(version, migration) in MIGRATIONS {
            assert!(
                statements(migration).any(|statement| statement
                    .contains("CREATE TABLE IF NOT EXISTS arcature_api_tokens")),
                "{version} does not create arcature_api_tokens"
            );
        }
    }
