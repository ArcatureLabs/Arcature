//! Remember-me schema migrations.
//!
//! One embedded migration per dialect, applied under a lock where the dialect
//! has one, with an `arcature_remember_tokens_schema_migrations` history
...
//!
//! This mirrors [`crate::jobs`]'s, [`crate::auth::session_store`]'s,
//! [`crate::tokens`]'s and the password-reset migrator rather than sharing
//! code with them, for the reason stated in the second: a generic migrator
//! parameterised over table name, history table, and lock would couple
//! subsystems that have no reason to change together, so that a schema change
//! in one could break another's migration.
...
/// # Errors
///
/// Returns [`RememberTokenError::Database`] if the database rejects a
...
mod tests {
    ...
    fn a_comment_that_mentions_the_separator_does_not_split_the_file() {
        // This copy of `statements` is a copy, not a call into a shared one,
        // so the line-not-substring rule has to be pinned here too. All three
        // bundled files open with a header comment quoting the sentinel.
    ...
    fn the_bundled_migration_never_stores_a_plaintext_secret() {
        // The whole property of this schema in one assertion: there are
        // digest columns and no column that could hold a secret itself. A
    ...
    fn the_expiry_column_has_no_null_state() {
        ...
        // the PostgreSQL migration. This is the credential that signs
        // somebody in weeks after their session ended, and an unbounded one
        // is a password that never changes and that its owner does not know
        // they have.
        for &(version, migration) in MIGRATIONS {
    ...
    fn the_previous_digest_is_nullable_because_a_fresh_token_has_none() {
        // The inverse of the test above, and the reason it cannot simply
        // assert `NOT NULL` on every column. A token that has never rotated
        // has no retired secret, and `NULL` is how the schema says so; a
        // `NOT NULL` here would force `issue` to invent a digest that no
        // secret hashes to.
        for &(version, migration) in MIGRATIONS {
            let line = migration
                .lines()
    ...
    fn the_subject_is_indexed_so_the_theft_cascade_is_not_a_scan() {
        // "Sign out everywhere" and the theft cascade both delete by subject,
        // and the cascade runs at the worst possible moment: somebody's
        // credential has just been used by somebody else.
        for &(version, migration) in MIGRATIONS {
            assert!(
