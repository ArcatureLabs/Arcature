//! Notification inbox schema migrations.
//!
//! One embedded migration per dialect, applied under a lock where the dialect
//! has one, with an `arcature_notifications_schema_migrations` history table.
...
//!
//! This mirrors [`crate::tokens`]'s, [`crate::jobs`]'s, and
//! [`crate::auth::session_store`]'s migrators rather than sharing code with
//! them, for the reason stated in each: a generic migrator parameterised over
//! table name, history table, and lock would couple subsystems that have no
//! reason to change together, so a schema change in one could break another's
//! migration.

use sqlx::{Executor, Row};

use super::channel::NotificationError;
use super::dialect::{NotificationDb, NotificationPool, sql};

/// The connection type of the dialect this build speaks.
type Conn = <NotificationDb as sqlx::Database>::Connection;
...
const MIGRATIONS: &[(&str, &str)] = &[("0001_notifications", sql::SCHEMA)];

/// Apply all pending migrations over the pool.
///
/// # Errors
///
/// Returns [`NotificationError::Database`] if the database rejects a statement
/// or the connection fails.
pub(super) async fn apply(pool: &NotificationPool) -> Result<(), NotificationError> {
...
/// them if it has one.
async fn apply_on(conn: &mut Conn) -> Result<(), NotificationError> {
...
async fn apply_pending(conn: &mut Conn) -> Result<(), NotificationError> {
...
mod tests {
    ...
    #[test]
    fn the_bundled_migration_creates_the_notifications_table() {
        ...
        for &(version, migration) in MIGRATIONS {
            assert!(
                statements(migration).any(|statement| statement
                    .contains("CREATE TABLE IF NOT EXISTS arcature_notifications")),
                "{version} does not create arcature_notifications"
            );
        }
    }

    #[test]
    fn every_row_is_scoped_by_its_recipient() {
        // The column that makes one recipient's inbox unreachable from
        // another's. Every statement in `dialect` names it in its `WHERE`,
        // so a schema without it would compile and then fail at the first
        // query -- and a schema that made it nullable would let a row exist
        // that no recipient owns.
        for &(version, migration) in MIGRATIONS {
            let key_line = migration
                .lines()
                .find(|line| line.trim_start().starts_with("notifiable_key"))
                .unwrap_or_else(|| panic!("{version} declares no notifiable_key column"));
            assert!(
                key_line.contains("NOT NULL"),
                "{version} allows an unowned notification: {key_line}"
            );
        }
    }

    #[test]
    fn unread_is_the_absence_of_a_timestamp() {
        // `read_at` must stay nullable. A `NOT NULL` column would force a
        // sentinel value for "unread", and every reader would then have to
        // know which value that is; see the header of the PostgreSQL
        // migration.
        for &(version, migration) in MIGRATIONS {
            let read_line = migration
                .lines()
                .find(|line| line.trim_start().starts_with("read_at"))
                .unwrap_or_else(|| panic!("{version} declares no read_at column"));
            assert!(
                !read_line.contains("NOT NULL"),
                "{version} cannot represent an unread notification: {read_line}"
            );
        }
    }

    #[test]
    fn the_inbox_read_is_indexed() {
        // Both reads this table exists to serve are keyed on the recipient:
        // the inbox listing and the unread badge. Without the indexes each
        // becomes a scan whose cost grows with everyone else's notifications,
        // which is exactly the shape of slowdown nobody notices until the
        // table is large.
        for &(version, migration) in MIGRATIONS {
            assert!(
                migration.contains("arcature_notifications_inbox_idx"),
                "{version} does not index the inbox read"
            );
            assert!(
                migration.contains("arcature_notifications_unread_idx"),
                "{version} does not index the unread badge"
            );
        }
    }
}
