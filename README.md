...
| `arc make:<kind> <name>` | Generate an artifact. 19 kinds: module, controller, model, migration, request, resource, policy, service, job, event, listener, middleware, command, page, test, factory, seeder, notification, mail. `module` writes a directory of four files; the rest write one apiece. |
