...
//! struct (the static `NAME` for `arc services` / `arc check` inspection)
//! plus a `POLICY_FOR` const recording the resource type the policy
...
pub fn policy(attr: TokenStream, item: TokenStream) -> MacroResult {
    ...
    // The model type name -- for `arc check` inspection. The model path
    // (e.g. `Link`) is recorded as a string so `arc services` can show
    // which resource type each policy authorizes for.
    ...
    Ok(quote! {
        ...
        impl #impl_generics #struct_name #ty_generics #where_clause {
            /// The resource type name this policy authorizes for (for
            /// `arc check` inspection). Generated by `#[policy(Model)]`.
